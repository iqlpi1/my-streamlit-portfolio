...
# 사용할 경우 CSS 파일 생성 필요
# 현재 오류의 원인일 가능성이 낮지만, 만약을 위해 주석 처리 상태 유지
# try:
#     local_css("style.css")
# except:
#     pass
...
# --- Plotly 차트 생성 함수 캐싱 적용 ---
# 동일한 입력에 대해 차트를 다시 생성하지 않도록 캐싱하여 성능 향상 및 오류 감소
@st.cache_data
def create_grades_bar_chart(grades_df):
    fig = px.bar(
        grades_df, 
        x='과목명', 
        y='성적점수',
...
@st.cache_data
def create_skills_radar_chart(skills, current_skills, target_skills):
    fig = go.Figure()
...
@st.cache_data
def create_exam_prep_chart(exam_df):
    fig = px.bar(
        exam_df, 
        x='시험 과목', 
        y='준비 상태 (%)',
...
@st.cache_data
def create_fitness_progress_chart(fitness_df):
    fig = px.line(
        fitness_df, 
        x='체력 항목', 
        y='달성율',
...
@st.cache_data
def create_development_scatter_chart(development_df):
    fig = px.scatter(
        development_df,
        x='중요도',
        y='현재 수준',
...
# 학업 정보 페이지
    ...
    with col_grade2:
        # 캐싱된 함수 호출 및 use_container_width=True 제거
        fig_grades = create_grades_bar_chart(grades_df)
        st.plotly_chart(fig_grades) # use_container_width=True 제거
...
# 미래 계획 페이지
    ...
    # 타임라인 시각화 (캐싱된 함수 호출 및 use_container_width=True 제거)
    fig_timeline = create_timeline_chart(timeline_df)
    st.plotly_chart(fig_timeline) # use_container_width=True 제거
...
# 역량 분석 페이지 (이전 '미래 계획' 섹션 하단에 있었던 내용을 별도 메뉴로 분리)
elif menu == "📊 역량 분석":
    st.title("📊 역량 분석")
    st.markdown("---")
    ...
    # 캐싱된 함수 호출 및 use_container_width=True 제거
    fig_radar = create_skills_radar_chart(skills, current_skills, target_skills)
    st.plotly_chart(fig_radar) # use_container_width=True 제거
    ...
    # 캐싱된 함수 호출 및 use_container_width=True 제거
    fig_exam_prep = create_exam_prep_chart(exam_df)
    st.plotly_chart(fig_exam_prep) # use_container_width=True 제거
    ...
    # 캐싱된 함수 호출 및 use_container_width=True 제거
    fig_fitness_progress = create_fitness_progress_chart(fitness_df)
    st.plotly_chart(fig_fitness_progress) # use_container_width=True 제거
    ...
    # 캐싱된 함수 호출 및 use_container_width=True 제거
    fig_development_scatter = create_development_scatter_chart(development_df)
    st.plotly_chart(fig_development_scatter) # use_container_width=True 제거
